...
const Brightness = ({ route,navigation }) => {
  ...
  const handleSnapshot = async () => {
         ...
         DeviceEventEmitter.emit("Brightness", {Img});
         navigation.goBack();
         };
  ...
  return (
    ...
    <View style={{flex:1,backgroundColor:'black',marginTop:200}}>
      ...
      <View style={{ marginBottom:50,width:380,borderTopWidth:2,borderTopColor:'pink',paddingTop:40,alignItems:"center"}}>
        <FontAwesome5 name='sun' size={25} color="white" style={{position:'absolute',right:330,marginTop:35}}/>
        <Slider
          step={0.1}
          minimumValue={0.1}
          maximumValue={2.0}
